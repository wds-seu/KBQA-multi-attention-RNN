    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            for r in one_relation:
                pattern = "[A-Z]"
                r = re.sub(pattern, lambda x: "_" + x.group(0), r)
                for w in r.split('_'):
...
def gloveEmbedding(embedding_filepath):
    ...
    with codecs.open(embedding_filepath,encoding='utf-8') as fin:
...
def process(data, relation, relation_all, question_dict, relation_dict, relation_all_dict):
    # 针对每一个question，得到了question的dict序号，relation和relation_all的正负例序号，label[-1,1]，每一个正例对应的负例个数计数
    ...
    for test_index,one_data in enumerate(data):
        ...
        for index in range(min(config.getint('pre', 'question_maximum_length'), len(question))):
            ...
            # one_question_feature: 按question中的word 转化成 question_dict中的序号
...
if __name__ == '__main__':
    ...
    # 把glove全部读入。
    # glove_dict(dict): #UNK:0 the:1 ,:2 ... 共4000001个元素,多一个#UNK
    # glove_embedding(dict): the:300d ,:300d ... 共4000000个元素
    # glove_emd_matrix(list): 300d 300d ... 共4000001个元素
    print('Embedding...')
    glove_dict, glove_embedding, glove_emd_matrix = gloveEmbedding(config.get('pre', 'embedding_filepath'))

    # 把relation全部读入。
    # relation(list): NONE parentOrganisation successor ...共3428123个元素
    # relation_all(list):  ['N', 'O', 'N', 'E'] ['parent', 'Organisation'] ...共3428123个元素,将relation word按首字母分开
    # relation_dict(dict): #UNK:0 NONE:1 parentOrganisation:2 ... relation的dict
    # relation_all_dict(dict): 同glove_dict
    print('Relations....')
    ...
    # relation_emd_matrix(ndarray): 300d 300d 300d ... 按relation_dict的顺序
    # relation_all_emd_matrix: 同glove_emd_matrix
    ...
    # 把data全部读入。
    # question_dict(dict)：#UNK:0 <e>:1 name:2 ... question中的word的dict
    # question_emd_matrix(ndarray): 300d 300d 300d ... 按question中的word顺序
    print('Data...')
